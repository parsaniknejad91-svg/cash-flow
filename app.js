...
function resetForm(close=true){editingId.value="";txName.value="";txAmount.value="";txCurrency.value="IRR";txDate.value=todayJalali();txDescription.value="";transactionDialogTitle.textContent="ثبت تراکنش جدید";setEntryType("in");if(close&&transactionDialog.open)transactionDialog.close()}
...
createUserBtn.addEventListener("click",async()=>{
  ...
  if(!full_name||!username||password.length<6){
    ...
    createUserMessage.textContent="نام، نام کاربری و رمز حداقل ۶ کاراکتری را کامل وارد کنید.";
    return;
  }

  if(!/^[a-z0-9._-]{3,32}$/.test(username)){
    createUserMessage.classList.add("error");
    createUserMessage.textContent="نام کاربری فقط با حروف انگلیسی، عدد، نقطه، خط تیره یا زیرخط و بین ۳ تا ۳۲ کاراکتر باشد.";
    return;
  }

  createUserBtn.disabled=true;
  createUserBtn.textContent="در حال ساخت...";
  ...
  try{
    ...
    if(!data?.ok)throw new Error(data?.error||"ساخت کاربر انجام نشد.");
    ...
    createUserMessage.textContent=`کاربر ${data.username} با موفقیت ساخته شد.`;
    ...
    console.error(err);
    createUserMessage.classList.add("error");
    let msg=err?.context?.json?.error||err?.message||String(err);
    if(/already|registered|exists/i.test(msg))msg="این نام کاربری قبلاً وجود دارد.";
    createUserMessage.textContent=msg;
  }finally{
    createUserBtn.disabled=false;
    createUserBtn.textContent="افزودن کاربر";
  }
});
